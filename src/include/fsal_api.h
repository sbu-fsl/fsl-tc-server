...
struct fsal_ops {
   ...
   * @return FSAL status.
   */
  fsal_status_t (*create_export)(struct fsal_module *fsal_hdl, void *parse_node,
                                 struct config_error_type *err_type,
   ...
   */
  void (*fsal_extract_stats)(struct fsal_module *const fsal_hdl, void *iter);
...
struct export_ops {
   ...
   * filesystem.  If FSALs are stacked, then the super-FSAL may want to pass
   * this through to the sub-FSAL to get the name, or add the sub-FSAL's name
   * onto it's own name.
   *
   ...
   *
   * This function is called when the export is unexported.  It should release
   * any working data that is not necessary when unexported, but not free the
   * export itself, as there are still references to it.
   *
   ...
   * attributes of interest. ATTR_ACL SHOULD NOT be requested and need not be
   * provided. If not all the requested attributes can be provided, this method
   * MUST return an error unless the ATTR_RDATTR_ERR bit was set in the
   * request_mask.
   *
   ...
   * @return FSAL types.
   */
  fsal_status_t (*get_quota)(struct fsal_export *exp_hdl, const char *filepath,
                             int quota_type, int quota_id, fsal_quota_t *quota);
   ...
   * @return FSAL types.
   */
  fsal_status_t (*set_quota)(struct fsal_export *exp_hdl, const char *filepath,
                             int quota_type, int quota_id, fsal_quota_t *quota,
...
struct fsal_obj_ops {
   ...
   */
  fsal_status_t (*copy)(struct fsal_obj_handle *src_hdl, uint64_t src_offset,
   ...
   * @return FSAL status.
   */
  fsal_status_t (*lookup)(struct fsal_obj_handle *dir_hdl, const char *path,
                          struct fsal_obj_handle **handle,
                          struct attrlist *attrs_out);
   ...
   * @return FSAL status.
   */
  fsal_status_t (*mkdir)(struct fsal_obj_handle *dir_hdl, const char *name,
   ...
   * group attributes if they are the same as the op_ctx->cred.
   *
   * If the node type has rawdev info, then @a attrs_in MUST have the rawdev
   * field set.
   *
   * The caller is expected to invoke fsal_release_attrs to release any
   * resources held by the set attributes. The FSAL layer MAY have added an
   ...
   * @return FSAL status.
   */
  fsal_status_t (*mknode)(struct fsal_obj_handle *dir_hdl, const char *name,
   ...
   * @return FSAL status.
   */
  fsal_status_t (*symlink)(struct fsal_obj_handle *dir_hdl, const char *name,
                           const char *link_path, struct attrlist *attrs_in,
                           struct fsal_obj_handle **new_obj,
                           struct attrlist *attrs_out);
  /**@}*/
   ...
   * @return FSAL status.
   */
  fsal_status_t (*seek)(struct fsal_obj_handle *obj_hdl, struct io_info *info);
  /**
   * @brief IO Advise
   *
  ...
  fsal_status_t (*list_ext_attrs)(struct fsal_obj_handle *obj_hdl,
                                  ...
                                  unsigned int *nb_returned, int *end_of_list);
   ...
   * FSAL_NO_CREATE. Some FSALs may still have to pass a mode on a create call
   * for exclusive, and even with FSAL_NO_CREATE, and empty set of attributes
   * MUST be passed.
   *
   ...
   * @return FSAL status.
   */
  fsal_status_t (*open2)(struct fsal_obj_handle *obj_hdl, struct state_t *state,
   ...
   * is expected to handle properly bypassing or not share reservations.  This
   *is an (optionally) asynchronous call.  When the I/O is complete, the done
   ...
   *
   * This is an (optionally) asynchronous call.  When the I/O is complete, the
   ...
   * @return FSAL status.
   */
  fsal_status_t (*seek2)(struct fsal_obj_handle *obj_hdl, struct state_t *state,
                         struct io_info *info);
  /**
   * @brief IO Advise
   *
  ...
  bool (*is_referral)(struct fsal_obj_handle *obj_hdl, struct attrlist *attrs,
                      bool cache_attrs);
...
struct fsal_pnfs_ds_ops {
   ...
   */
  nfsstat4 (*permissions)(struct fsal_pnfs_ds *const pds, struct svc_req *req);
  /**@}*/
...
struct fsal_dsh_ops {
   ...
   */
  nfsstat4 (*write_plus)(
      struct fsal_ds_handle *const ds_hdl, struct req_op_context *const req_ctx,
      const stateid4 *stateid, const offset4 offset, const count4 write_length,
