...
struct fsal_ops {
	...
	/**
	 * @brief Max Size of the buffer needed for da_addr_body in
	 * getdeviceinfo
	 *
	 ...
	 * what the client can take return ~0UL. In any case the buffer
	 * allocated will not be bigger than client's requested maximum.
	 *
...
struct export_ops {
	 ...
	 *
	 * This function is used to find the name of the ultimate FSAL providing
	 * the filesystem.  If FSALs are stacked, then the super-FSAL may want
	 * to pass this through to the sub-FSAL to get the name, or add the
	 * sub-FSAL's name onto it's own name.
	 *
	 ...
	 *
	 * This function is called prior to unexporting an export. It should do
	 * any preparation that the export requires prior to being removed.
	 */
	 ...
	 *
	 * This function is called when the export is unexported.  It should
	 * release any working data that is not necessary when unexported, but
	 * not free the export itself, as there are still references to it.
	 *
	 ...
	 *
	 * The caller will set the request_mask in attrs_out to indicate the
	 * attributes of interest. ATTR_ACL SHOULD NOT be requested and need not
	 * be provided. If not all the requested attributes can be provided,
	 * this method MUST return an error unless the ATTR_RDATTR_ERR bit was
	 * set in the request_mask.
	 *
	 * Since this method instantiates a new fsal_obj_handle, it will be
	 * forced to fetch at least some attributes in order to even know what
	 * the object type is (as well as it's fileid and fsid). For this
	 * reason, the operation as a whole can be expected to fail if the
	 * attributes were not able to be fetched.
	 *
	 ...
	 *
	 * This function is the handler of the NFS4.1 FATTR4_FS_LAYOUT_TYPES
	 * file attribute. (See RFC)
	 *
	 * @param[in]  exp_hdl Filesystem to interrogate
	 ...
	 *
	 * This function is the handler of the NFS4.1 FATTR4_LAYOUT_BLKSIZE
	 * f-attribute.
	 *
	 * This is the preferred read/write block size.  Clients are requested
	 * (but don't have to) read and write in multiples.
	 *
	 * NOTE: The linux client only asks for this in blocks-layout, where
	 * this is the filesystem wide block-size. (Minimum write size and
	 * alignment)
	 *
	 * @param[in] exp_hdl Filesystem to interrogate
	 *
	 ...
	 * @param[in] exp_hdl Filesystem to interrogate
	 *
	 * @return The Maximum number of layout segments in a campound
	 * layoutget.
	 */
	 ...
	 * vector below. If FSAL has a const size, just return it here. If it is
	 * dependent on what the client can take return ~0UL. In any case the
	 * buffer allocated will not be bigger than client's requested maximum.
	 *
	 * @param[in] exp_hdl Filesystem to interrogate
	 *
	 ...
	 *
	 * This function is called by write and commit to match the commit
	 * verifier with the one returned on  write.
	 *
	 ...
	 *
	 * @param[in] exp_hdl               Export state_t will be associated
	 * with
	 ...
	 * Compound events
	 */
	fsal_status_t (*start_compound)(struct fsal_export *exp_hdl,
					void *data);
...
struct fsal_obj_ops {
	 ...
	 *
	 * Refcounting is required for all FSALs. An FSAL that will have
	 * FSAL_MDCACHE stacked on top need not handle this as FSAL_MDCACHE will
	 * handle it.
	 *
	 ...
	 * @param[in]  attrmask  Indicate which attributes the caller is
	 * interested in
	 ...
	 *
	 * Some FSALs are able to compute the cookie for a filename
	 * deterministically from the filename. They also have a defined order
	 * of entries in a directory based on the name (could be strcmp sort,
	 * could be strict alpha sort, could be deterministic order based on
	 * cookie - in any case, the dirent_cmp method will also be provided.
	 *
	 * The returned cookie is the cookie that can be passed as whence to
	 * FIND that directory entry. This is different than the cookie passed
	 * in the readdir callback (which is the cookie of the NEXT entry).
	 *
	 ...
	 * deterministically from the filename, there must also be a defined
	 * order of entries in a directory based on the name (could be strcmp
	 * sort, could be strict alpha sort, could be deterministic order based
	 * on cookie).
	 *
	 * Although the cookies could be computed, the caller will already have
	 * them and thus will provide them to save compute time.
	 *
	 ...
	 * resources held by the set attributes. The FSAL layer MAY have added
	 * an inherited ACL.
	 *
	 * The caller will set the request_mask in attrs_out to indicate the
	 * attributes of interest. ATTR_ACL SHOULD NOT be requested and need not
	 ...
	 * group attributes if they are the same as the op_ctx->cred.
	 *
	 * If the node type has rawdev info, then @a attrs_in MUST have the
	 * rawdev field set.
	 *
	 * The caller is expected to invoke fsal_release_attrs to release any
	 * resources held by the set attributes. The FSAL layer MAY have added
	 ...
	 * content lock.  This should return ERR_FSAL_NOT_OPENED if the global
	 ...
	 *
	 * This function compares two handles to see if they reference the same
	 * file
	 *
	...
	fsal_status_t (*getxattrs)(struct fsal_obj_handle *obj_hdl,
				   xattrname4 *xa_name, xattrvalue4 *xa_value);
	...
	fsal_status_t (*setxattrs)(struct fsal_obj_handle *obj_hdl,
				   setxattr_type4 sa_type, xattrname4 *xa_name,
	...
	fsal_status_t (*listxattrs)(struct fsal_obj_handle *obj_hdl,
				    ...
				    verifier4 *la_cookieverf, bool_t *lr_eof,
				    xattrlist4 *lr_names);
	 ...
	 *
	 * The open2 method also allows atomic create/setattr/open (just like
	 * the NFS v4 OPEN operation).
	 *
	 */
	 ...
	 * NFS v3 CREATE operation), in which case the FSAL must assure
	 * protection of any resources. If the file is being created, such
	 * protection is simple since no one else will have access to the object
	 * yet, however, in the case of an exclusive create, the common
	 * resources may still need protection.
	 *
	 ...
	 *
	 * At least the mode attribute must be set if createmode is not
	 * FSAL_NO_CREATE. Some FSALs may still have to pass a mode on a create
	 * call for exclusive, and even with FSAL_NO_CREATE, and empty set of
	 * attributes MUST be passed.
	 *
	 * If an open by name succeeds and did not result in Ganesha creating a
	 * file, the caller will need to do a subsequent permission check to
	 * confirm the open. This is because the permission attributes were not
	 ...
	 * set in the request_mask.
	 *
	 * Since this method may instantiate a new fsal_obj_handle, it will be
	 * forced to fetch at least some attributes in order to even know what
	 * the object type is (as well as it's fileid and fsid). For this
	 * reason, the operation as a whole can be expected to fail if the
	 * attributes were not able to be fetched.
	 ...
	 * @param[in,out] attrs_out         Optional attributes for newly
	 * created object
	 * @param[in,out] caller_perm_check The caller must do a permission
	 * check
	 *
	 * @return FSAL status.
	 */
	 ...
	 *
	 * This function supports changing the access mode of a share
	 * reservation and thus should only be called with a share state. The
	 * state_lock must be held.
	 *
	 * This MAY be used to open a file the first time if there is no need
	 * for open by name or create semantics. One example would be 9P lopen.
	 *
	 ...
	 *
	 * This function reads data from the given file. The FSAL must be able
	 *to perform the read whether a state is presented or not. This function
	 *also is expected to handle properly bypassing or not share
	 *reservations.  This is an (optionally) asynchronous call.  When the
	 *I/O is complete, the done callback is called with the results.
	 *
	 * @param[in]     obj_hdl	File on which to operate
	 ...
	 * perform the write whether a state is presented or not. This function
	 * also is expected to handle properly bypassing or not share
	 * reservations. Even with bypass == true, it will enforce a mandatory
	 * (NFSv4) deny_write if an appropriate state is not passed).
	 *
	 * The FSAL is expected to enforce sync if necessary.
	 *
	 * This is an (optionally) asynchronous call.  When the I/O is complete,
	 * the
	 * @a done_cb callback is called.
	 *
	 * @param[in]     obj_hdl       File on which to operate
	 * @param[in]     bypass        If state doesn't indicate a share
	 * reservation, bypass any non-mandatory deny write
	 * @param[in,out] done_cb	Callback to call when I/O is done
	 * @param[in,out] write_arg	Info about write, passed back in
	 * callback
	 * @param[in,out] caller_arg	Opaque arg from the caller for callback
	 */
	 ...
	 * differs from commit due to the need to interact with share
	 * reservations and the fact that the FSAL manages the state of "file
	 * descriptors". The FSAL must be able to perform this operation without
	 * being passed a specific state.
	 *
	 * @param[in] obj_hdl          File on which to operate
	 * @param[in] state            state_t to use for this operation
	 ...
	 * lock owner for each file (which may include use of a "file
	 * descriptor".
	 *
	 * @param[in]  obj_hdl          File on which to operate
	 * @param[in]  state            state_t to use for this operation
	 ...
	 *
	 * When the upper layers are ready to dispense with a state, this method
	 * is called to allow the FSAL to close any file descriptors or release
	 * any other resources associated with the state. A call to free_state
	 * should be assumed to follow soon.
	 *
	 * @param[in] obj_hdl    File on which to operate
	 * @param[in] state      state_t to use for this operation
	 *
	 ...
	 * ASYNC API functions.
	 *
	 * These are asyncronous versions of some of the API functions.  FSALs
	 * are expected to implement these, but the upper layers are not
	 * expected to call them.  Instead, they will be called by MDCACHE at
...
struct fsal_dsh_ops {
	 ...
	 * @param[in]  requested_length Length of read requested (and size of
	 * buffer)
	 ...
	 * operation, for validation
	 * @param[in]  offset           The offset at which to read
	 * @param[in]  write_length     Length of write requested (and size of
	 * buffer)
	 * @param[out] buffer           The buffer to which to store read data
