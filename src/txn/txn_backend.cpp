...
static const char* fstxn_backend_root = nullptr;
static const char* fstxn_txnfilename = nullptr;

static const char* ldbtxn_backend_root = nullptr;
static const char* ldbtxn_key_prefix = nullptr;
...
int ldbtxn_init(void) {
  leveldb::Options options;
  options.create_if_missing = true;
  leveldb::Status status = leveldb::DB::Open(options, ldbtxn_backend_root, &db);
  if (!status.ok()) {
    std::cerr << "Failed to intitialize leveldb" << std::endl;
    return -1;
  }
...
int ldbtxn_create_txn(uint64_t txn_id, struct TxnLog* txn) {
  ...
  leveldb::Status status =
      db->Put(leveldb::WriteOptions(), ldbtxn_get_key(txn_id), value);
  if (!status.ok()) {
...
int ldbtxn_get_txn(uint64_t txn_id, struct TxnLog* txn) {
  std::string value;
  leveldb::Status status =
      db->Get(leveldb::ReadOptions(), ldbtxn_get_key(txn_id), &value);
  if (!status.ok()) {
...
void ldbtxn_enumerate_txn(void (*callback)(struct TxnLog* txn)) {
  ...
  for (it->SeekToFirst();
       it->Valid() && it->key().starts_with(ldbtxn_key_prefix); it->Next()) {
    struct TxnLog txn;
    uint64_t txn_id =
        stoull(it->key().ToString().substr(strlen(ldbtxn_key_prefix)));
...
int ldbtxn_remove_txn(uint64_t txn_id) {
  leveldb::Status status =
      db->Delete(leveldb::WriteOptions(), ldbtxn_get_key(txn_id));
  if (!status.ok()) {
    std::cerr << "Failed to read txn to leveldb" << std::endl;
    return -1;
  }
...
void ldbtxn_shutdown(void) { delete db; }
...
int fstxn_create_txn(uint64_t txn_id, struct TxnLog* txn) {
  ...
  // write to a file
  fstream output(txnpath.c_str(), ios::out | ios::trunc | ios::binary);
...
void init_ldbtxn_backend(const char* root, const char* prefix,
  ...
  ldbtxn_backend_root = root;
  ldbtxn_key_prefix = prefix;
  *txn_backend = &ldbtxn_backend;
}

static struct txn_backend fstxn_backend = {.backend_init = fstxn_backend_init,
...
void init_fstxn_backend(const char* root, const char* filename,
                        struct txn_backend** txn_backend) {
  fstxn_backend_root = root;
  fstxn_txnfilename = filename;
