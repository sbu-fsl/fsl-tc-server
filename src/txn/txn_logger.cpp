	...
	for (int i = 0; i < txn_log->num_files; i++) {
		...
		if (txn_log->created_file_ids[i].flags) {
			...
			if (ENABLE_NORMAL_TEXT_LOGGING &&
				...
				char* hex_uuid = bytes_to_hex(
				    txn_log->created_file_ids[i].data);
				file_id->set_id((const char*)hex_uuid);
			}
			file_id->set_type(get_file_type_txn(
...
void write_unlink_txn(struct TxnLog* txn_log, TransactionLog* txn_log_obj) {
	...
	for (int i = 0; i < txn_log->num_unlinks; i++) {
		...
		unlink_obj->set_original_path(
		    txn_log->created_unlink_ids[i].original_path);
		unlink_obj->set_backup_name(
...
void write_symlink_txn(struct TxnLog* txn_log, TransactionLog* txn_log_obj) {
	...
	for (int i = 0; i < txn_log->num_unlinks; i++) {
		VSymlinkTxn_Symlink* symlink_obj =
		    symlink_txn->add_linked_objs();
		symlink_obj->set_src_path(
		    txn_log->created_symlink_ids[i].src_path);
		symlink_obj->set_dst_path(
		    txn_log->created_symlink_ids[i].dst_path);
	}
}
...
void write_rename_txn(struct TxnLog* txn_log, TransactionLog* txn_log_obj) {
	...
	for (int i = 0; i < txn_log->num_renames; i++) {
		...
		rename_obj->set_src_path(
		    txn_log->created_rename_ids[i].src_path);
		rename_obj->set_dst_path(
		    txn_log->created_rename_ids[i].dst_path);
		rename_obj->set_is_directory(
		    txn_log->created_rename_ids[i].is_directory);
		FileID* src_fileid = rename_obj->mutable_src_fileid();
		src_fileid->set_id(
		    txn_log->created_rename_ids[i].src_fileid.data);
		src_fileid->set_type(get_file_type_txn(
		...
		dst_fileid->set_type(get_file_type_txn(
...
void read_create_txn(TransactionLog* txn_log_obj, struct TxnLog* txn_log) {
	...
	if (txn_log_obj->has_creates()) {
		...
		for (int i = 0; i < create_txn.created_files_size(); i++) {
			...
			created_files[num_created_files].data =
			    file_id.id().c_str();
			if (file_id.has_type())
...
void read_unlink_txn(TransactionLog* txn_log_obj, struct TxnLog* txn_log) {
	...
	if (txn_log_obj->has_unlinks()) {
		...
		for (int i = 0; i < unlink_txn.unlinked_objs_size(); i++) {
			VUnlinkTxn_Unlink unlink_obj =
			    unlink_txn.unlinked_objs(i);
...
void read_symlink_txn(TransactionLog* txn_log_obj, struct TxnLog* txn_log) {
	...
	if (txn_log_obj->has_symlinks()) {
		...
		for (int i = 0; i < symlink_txn.linked_objs_size(); i++) {
			VSymlinkTxn_Symlink link_obj =
			    symlink_txn.linked_objs(i);
...
void read_rename_txn(TransactionLog* txn_log_obj, struct TxnLog* txn_log) {
	...
	if (txn_log_obj->has_renames()) {
		...
		for (int i = 0; i < rename_txn.renamed_objs_size(); i++) {
			VRenameTxn_Rename rename_obj =
			    rename_txn.renamed_objs(i);
...
int write_txn_log(struct TxnLog* txn_log, const char* inputdir) {
	...
	if (strcmp(inputdir, "") == 0) {
		txn_log_file_name =
		    "/var/log/txn_" + to_string(txn_log->txn_id);
		if (ENABLE_NORMAL_TEXT_LOGGING) {
		...
		if (ENABLE_NORMAL_TEXT_LOGGING) {
			txn_log_txt_file_name +=
			    "/normal_txn_" + to_string(txn_log->txn_id);
		}
	}
	fstream input(txn_log_file_name.c_str(), ios::in | ios::binary);
	if (!input) {
		cout << txn_log_file_name
	...
	if (ENABLE_NORMAL_TEXT_LOGGING) {
		fstream normal_output(txn_log_txt_file_name.c_str(),
				      ios::out | ios::trunc);
 ...
 */
int iterate_txn_logs(
    const char* log_dir,
    // Ming: consider change |log_processor| to std::function so that it can be
    // either a function pointer, a lambda, or a functor object.
    int (*log_processor)(struct TxnLog* log)) {
	struct dirent* d;
	int count = 0;
			...
			if (t != nullptr) {
				...
				if ((txn_id <= LONG_MIN) ||
					...
					struct TxnLog* txn_log_ret =
					    (struct TxnLog*)calloc(
						1, sizeof(struct TxnLog));
					txn_log_ret =
					    read_txn_log((int)txn_id, log_dir);
