...
std::vector<lock_request_t> make_lock_requests(
  ...
  for (char c : std::string("abcdef")) {
    ...
    threads.push_back(std::thread([&lm, path]() {
      ...
      for (int i = 0; i < 100; i++) {
        ...
        ASSERT_TRUE(lh2);
        std::this_thread::sleep_for(std::chrono::milliseconds(rand(rng)));
        unlock_handle(lh2);
      }
    }));
  }
...
TEST(LockManagerTest, LockSets) {
  ...
  auto lr = make_lock_requests({{"test", false},
                                {"test/a", false},
                                {"test/b", false},
                                {"test/b/c", false},
                                {"test/c", true}});
  ...
  auto lr3 = make_lock_requests(
      {{"test", false}, {"test/b/c", false}, {"test/c", true}});
  ...
  auto lr6 = make_lock_requests({{"test", true},
                                 {"test/a", true},
                                 {"test/b", true},
                                 {"test/b/c", true},
                                 {"test/c", true}});
