...
protected:
  ...
  int compareStructsCommon(TxnLog txn_log, TxnLog *txn_log_ret) {
    if (txn_log_ret->txn_id != txn_log.txn_id)
      return 1;
  ...
  int compareStructsFileID(TxnLog txn_log, TxnLog *txn_log_ret) {
    if (txn_log_ret->num_files != txn_log.num_files)
      return 1;
    ...
    if (created_files_ret->file_type != created_files->file_type)
      return 1;
    if (created_files_ret->flags != created_files->flags)
      return 1;
    if (id.compare(created_files_ret->data) != 0)
      return 1;
    return 0;
  }
  int compareStructsUnlinkId(TxnLog txn_log, TxnLog *txn_log_ret) {
    if (txn_log_ret->num_unlinks != txn_log.num_unlinks)
      return 1;
  ...
  int compareStructsSymlinkId(TxnLog txn_log, TxnLog *txn_log_ret) {
    if (txn_log_ret->num_symlinks != txn_log.num_symlinks)
      return 1;
  ...
  int compareStructsRenameId(TxnLog txn_log, TxnLog *txn_log_ret) {
    if (txn_log_ret->num_renames != txn_log.num_renames)
      return 1;
