...
/* xattrs.c
 * CoWFS object (file|dir) handle object extended attributes
 */
...
int print_cowfshandle(struct fsal_obj_handle *obj_hdl, caddr_t buffer_addr,
...
static struct fsal_xattr_def xattr_list[] = {
	{"cowfshandle", print_cowfshandle, NULL, XATTR_FOR_ALL | XATTR_RO, NULL},
};
				 ...
				 unsigned int *p_nb_returned, int *end_of_list)
{
	...
	struct cowfs_fsal_obj_handle *obj_handle = NULL;
	int fd = -1;
	fsal_errors_t fe;
	...
	fd = (obj_hdl->type == DIRECTORY) ?
	    cowfs_fsal_open(obj_handle, O_DIRECTORY, &fe) :
	    cowfs_fsal_open(obj_handle, O_RDWR, &fe);
	if (fd < 0)
					...
					unsigned int *pxattr_id)
{
	...
	if (!found) {
		...
		fd = cowfs_fsal_open(obj_handle, openflags, &fe);
		if (fd < 0)
			return fsalstat(fe, -fd);
					   ...
					   size_t *p_output_size)
{
	...
	for (index = 0; index < XATTR_COUNT; index++) {
		if (do_match_type(xattr_list[index].flags,
			...
			return cowfs_getextattr_value_by_id(obj_hdl, index,
							  buffer_addr,
							  buffer_size,
							  p_output_size);
		}
					 ...
					 size_t buffer_size)
{
	...
	return cowfs_setextattr_value(obj_hdl, name, buffer_addr,
				    buffer_size, false);
}
