...
{
	struct cowfs_filesystem *cowfs_fs = hdl->obj_handle.fs->private_data;

	return cowfs_open_by_handle(cowfs_fs, hdl->handle, openflags, fsal_error);
}

/**
 * @brief Create a CoWFS OBJ handle
 *
 * @param[in] dirfd	FD for dir containing new handle
 * @param[in] fh	CoWFS FH for new handle
 * @param[in] fs	FileSystem containing new handle
 * @param[in] stat	stat(2) resutls for new handle
 * @param[in] dir_fh	CoWFS FH for dir containing new handle
 * @param[in] path	Path to new handle
 * @param[in] exp_hdl	Export containing new handle
 * @return CoWFS OBJ handle on success, NULL on failure
 */
struct cowfs_fsal_obj_handle *alloc_handle(int dirfd,
					 vfs_file_handle_t *fh,
					 struct fsal_filesystem *fs,
					 struct stat *stat,
					 struct cowfs_fsal_obj_handle *dir_fh,
					 const char *path,
					 struct fsal_export *exp_hdl)
{
	struct cowfs_fsal_export *myself =
	    container_of(exp_hdl, struct cowfs_fsal_export, export);
	struct cowfs_fsal_obj_handle *hdl;
	struct cowfs_fsal_module *my_module = container_of(
			exp_hdl->fsal, struct cowfs_fsal_module, module);

	hdl = cowfs_sub_alloc_handle();
	...
	if (hdl->obj_handle.type == REGULAR_FILE) {
	...
	} else if (cowfs_unopenable_type(hdl->obj_handle.type)) {
		...
		if (dir_fh == NULL) {
			...
			retval =
			    cowfs_fd_to_handle(dirfd, hdl->obj_handle.fs, fh);
...
#ifdef CoWFS_NO_MDCACHE
	hdl->obj_handle.state_hdl = cowfs_state_locate(&hdl->obj_handle);
#endif /* CoWFS_NO_MDCACHE */
	cowfs_handle_ops_init(&hdl->obj_handle.obj_ops);
	hdl->obj_handle.obj_ops = &my_module->handle_ops;
	if (cowfs_sub_init_handle(myself, hdl, path) < 0)
		goto spcerr;
...
static fsal_status_t lookup_with_fd(struct cowfs_fsal_obj_handle *parent_hdl,
				    ...
				    struct attrlist *attrs_out)
{
	...
	if (xfsal || cowfs_name_to_handle(dirfd, fs, path, fh) < 0) {
		retval = errno;
		if (((retval == ENOTTY) ||
		     (retval == EOPNOTSUPP) ||
		     (retval == ENOTSUP) ||
			 ...
			 * this FSAL (XFS or CoWFS). Need to invent a handle.
			 ...
			 */
			LogDebug(COMPONENT_FSAL,
				 "cowfs_name_to_handle %s, inventing FSAL %s handle for FSAL %s filesystem %s",
			...
			retval = cowfs_encode_dummy_handle(fh, fs);
	...
	hdl = alloc_handle(dirfd, fh, fs, &stat, parent_hdl, path,
			   op_ctx->fsal_export);
	...
	*handle = &hdl->obj_handle;
	LogDebug(COMPONENT_FSAL, "handle %p abs %s\n", *handle, (*handle)->absolute_path);
			    ...
			    struct attrlist *attrs_out)
{
	...
	*handle = NULL;		/* poison it first */
	parent_hdl =
	    container_of(parent, struct cowfs_fsal_obj_handle, obj_handle);
	LogDebug(COMPONENT_FSAL, "lookup in cowfs %s\n", path);
	...
	dirfd = cowfs_fsal_open(parent_hdl, O_PATH | O_NOACCESS, &fsal_error);
...
static fsal_status_t makedir(struct fsal_obj_handle *dir_hdl,
			     ...
			     struct attrlist *attrs_out)
{
	struct cowfs_fsal_obj_handle *myself, *hdl;
	int dir_fd;
	struct stat stat;
	mode_t unix_mode;
	...
	*handle = NULL;		/* poison it */
	...
	myself = container_of(dir_hdl, struct cowfs_fsal_obj_handle, obj_handle);
	...
	dir_fd = cowfs_fsal_open(myself, flags, &status.major);
	if (dir_fd < 0) {
		LogFullDebug(COMPONENT_FSAL,
			     "cowfs_fsal_open returned %s",
			     strerror(-dir_fd));
	...
	if (retval < 0) {
		retval = errno;
		LogFullDebug(COMPONENT_FSAL,
			     "cowfs_stat_by_handle returned %s",
			     strerror(retval));
		status = posix2fsal_status(retval);
		goto direrr;
	}
	...
	fsal_restore_ganesha_credentials();
	retval =  cowfs_name_to_handle(dir_fd, dir_hdl->fs, name, fh);
	if (retval < 0) {
		retval = errno;
		status = posix2fsal_status(retval);
		goto fileerr;
			      ...
			      struct attrlist *attrs_out)
{
	...
	*handle = NULL;		/* poison it */
	...
	retval = cowfs_name_to_handle(dir_fd, myself->obj_handle.fs, name, fh);
	...
	hdl = alloc_handle(dir_fd, fh, myself->obj_handle.fs, &stat,
			   myself, name, op_ctx->fsal_export);
				 ...
				 bool refresh)
{
	...
	if (refresh) {		/* lazy load or LRU'd storage */
		retval = cowfs_readlink(myself, &fsal_error);
		if (retval < 0) {
			retval = -retval;
			goto hdlerr;
		}
...
static fsal_status_t linkfile(struct fsal_obj_handle *obj_hdl,
			      ...
			      const char *name)
{
	struct cowfs_fsal_obj_handle *myself, *destdir;
	int srcfd, destdirfd;
	int retval = 0;
	...
	if (obj_hdl->type == REGULAR_FILE &&
		...
		srcfd = cowfs_fsal_open(myself, flags, &fsal_error);
		if (srcfd < 0) {
			retval = -srcfd;
			fsal_error = posix2fsal_error(retval);
			LogDebug(COMPONENT_FSAL,
	...
	destdir =
	    container_of(destdir_hdl, struct cowfs_fsal_obj_handle, obj_handle);
	...
	destdirfd = cowfs_fsal_open(destdir, flags, &fsal_error);
				  ...
				  bool *eof)
{
	...
	dirfd = cowfs_fsal_open(myself, O_RDONLY | O_DIRECTORY, &status.major);
	if (dirfd < 0) {
		retval = -dirfd;
		status = posix2fsal_status(retval);
		goto out;
...
static fsal_status_t renamefile(struct fsal_obj_handle *obj_hdl,
				...
				const char *new_name)
{
	struct cowfs_fsal_obj_handle *olddir, *newdir, *obj;
	int oldfd = -1, newfd = -1;
	fsal_errors_t fsal_error = ERR_FSAL_NO_ERROR;
	int retval = 0;

	olddir =
	    container_of(olddir_hdl, struct cowfs_fsal_obj_handle, obj_handle);
	...
	oldfd = cowfs_fsal_open(olddir, O_PATH | O_NOACCESS, &fsal_error);
	if (oldfd < 0) {
		retval = -oldfd;
		goto out;
	}
	newdir =
	    container_of(newdir_hdl, struct cowfs_fsal_obj_handle, obj_handle);
	...
	obj = container_of(obj_hdl, struct cowfs_fsal_obj_handle, obj_handle);
	newfd = cowfs_fsal_open(newdir, O_PATH | O_NOACCESS, &fsal_error);
	if (newfd < 0) {
		retval = -newfd;
		goto out;
	}
	...
	if (retval < 0) {
		retval = errno;
		fsal_error = posix2fsal_error(retval);
	} else if (cowfs_unopenable_type(obj->obj_handle.type)) {
				      ...
				      fsal_errors_t *fsal_error)
{
	...
	struct cowfs_filesystem *cowfs_fs = myself->obj_handle.fs->private_data;
	int open_flags;
	...
	case CHARACTER_FILE:
	case BLOCK_FILE:
		cfd.fd = cowfs_open_by_handle(cowfs_fs,
	...
	case REGULAR_FILE:
		...
		if (((flags & FSAL_O_ANY) != 0 &&
			...
			cfd.fd = cowfs_fsal_open(myself, open_flags, fsal_error);
			if (cfd.fd < 0) {
				LogDebug(COMPONENT_FSAL,
					 "Failed with %s open_flags 0x%08x",
				 ...
				 const char *name)
{
	...
	fd = cowfs_fsal_open(myself, O_PATH | O_NOACCESS, &fsal_error);
	if (fd < 0) {
		retval = -fd;
		goto out;
	}
...
{
	const struct cowfs_fsal_obj_handle *myself;
...
static void release(struct fsal_obj_handle *obj_hdl)
{
	...
	if (type == REGULAR_FILE) {
		...
		st = cowfs_close_my_fd(&myself->u.file.fd);
	...
	if (type == SYMBOLIC_LINK) {
		...
		cowfs_state_release(&key);
	} else if (cowfs_unopenable_type(type)) {
...
void cowfs_handle_ops_init(struct fsal_obj_ops *ops)
{
	...
	ops->getattrs = cowfs_getattr2;
	ops->link = linkfile;
	ops->rename = renamefile;
	ops->unlink = file_unlink;
	ops->close = cowfs_close;
	ops->handle_to_wire = handle_to_wire;
	ops->handle_to_key = handle_to_key;
	ops->open2 = cowfs_open2;
	ops->reopen2 = cowfs_reopen2;
	ops->read2 = cowfs_read2;
	ops->write2 = cowfs_write2;
	ops->copy = cowfs_copy;
	ops->commit2 = cowfs_commit2;
#ifdef F_OFD_GETLK
	ops->lock_op2 = cowfs_lock_op2;
#endif
	ops->setattr2 = cowfs_setattr2;
	ops->close2 = cowfs_close2;

	/* xattr related functions */
	ops->list_ext_attrs = cowfs_list_ext_attrs;
	ops->getextattr_id_by_name = cowfs_getextattr_id_by_name;
	ops->getextattr_value_by_name = cowfs_getextattr_value_by_name;
	ops->getextattr_value_by_id = cowfs_getextattr_value_by_id;
	ops->setextattr_value = cowfs_setextattr_value;
			      ...
			      struct attrlist *attrs_out)
{
	...
	*handle = NULL;	/* poison it */
	...
	retval = cowfs_fd_to_handle(dir_fd, fs, fh);
	...
	*handle = &hdl->obj_handle;
	strcat((*handle)->absolute_path, path);
				...
				struct attrlist *attrs_out)
{
	...
	*handle = NULL;		/* poison it first */
	...
	status = cowfs_check_handle(exp_hdl, hdl_desc, &fs, fh, &dummy);
	...
	if (dummy) {
		...
		fd = cowfs_open_by_handle(fs->private_data, fh, flags,
					&fsal_error);
