...
/* file.c
 * File I/O methods for CoWFS module
 */
...
fsal_status_t cowfs_open_my_fd(struct cowfs_fsal_obj_handle *myself,
			     fsal_openflags_t openflags,
			     int posix_flags,
			     struct cowfs_fd *my_fd)
{
	int fd;
	...
	fd = cowfs_fsal_open(myself, posix_flags, &fsal_error);
...
static fsal_status_t cowfs_open_func(struct fsal_obj_handle *obj_hdl,
				   fsal_openflags_t openflags,
				   struct fsal_fd *fd)
{
	...
	myself = container_of(obj_hdl, struct cowfs_fsal_obj_handle, obj_handle);

	fsal2posix_openflags(openflags, &posix_flags);

	return cowfs_open_my_fd(myself, openflags, posix_flags,
			        (struct cowfs_fd *)fd);
}
...
static fsal_status_t cowfs_close_func(struct fsal_obj_handle *obj_hdl,
				      struct fsal_fd *fd)
{
	return cowfs_close_my_fd((struct cowfs_fd *)fd);
}
...
fsal_status_t cowfs_close(struct fsal_obj_handle *obj_hdl)
{
	...
	status = cowfs_close_my_fd(&myself->u.file.fd);
				...
				struct state_t *related_state)
{
	...
	state = init_state(gsh_calloc(1, sizeof(struct cowfs_state_fd),
			   exp_hdl, state_type, related_state);

	my_fd = &container_of(state, struct cowfs_state_fd, state)->fd;
...
/**
 * @brief free a cowfs_state_fd structure
 *
 * @param[in] exp_hdl  Export state_t will be associated with
 * @param[in] state    Related state if appropriate
 *
 */
void cowfs_free_state(struct fsal_export *exp_hdl, struct state_t *state)
{
	struct cowfs_state_fd *state_fd =
		container_of(state, struct cowfs_state_fd, state);
	struct cowfs_fd *my_fd = &state_fd->fd;
			...
			struct fsal_obj_handle *dupe_hdl)
{
	...
	if (orig_hdl->type == REGULAR_FILE &&
		 ...
		 */
		struct cowfs_fsal_obj_handle *orig, *dupe;
...
static fsal_status_t fetch_attrs(struct cowfs_fsal_obj_handle *myself,
				 int my_fd, struct attrlist *attrs)
{
	...
	case FIFO_FILE:
	case DIRECTORY:
		retval = cowfs_stat_by_handle(my_fd, &stat);
		func = "cowfs_stat_by_handle";
		break;
...
static fsal_status_t cowfs_open2_by_handle(struct fsal_obj_handle *obj_hdl,
					 struct state_t *state,
					 fsal_openflags_t openflags,
...
{
	fsal_status_t status = {0, 0};
	struct cowfs_fd *my_fd = NULL;
	int posix_flags = 0;
	bool truncated;
	struct cowfs_fsal_obj_handle *myself =
		container_of(obj_hdl, struct cowfs_fsal_obj_handle, obj_handle);

	if (state != NULL)
		my_fd = &container_of(state, struct cowfs_state_fd,
				      state)->fd;
	...
	if (my_fd->openflags != FSAL_O_CLOSED) {
		cowfs_close_my_fd(my_fd);
	}
	status = cowfs_open_my_fd(myself, openflags, posix_flags, my_fd);
			...
			bool *caller_perm_check)
{
	...
	fsal_status_t status = {0, 0};
	struct cowfs_fd *my_fd = NULL;
	struct cowfs_fsal_obj_handle *myself, *hdl = NULL;
	struct stat stat;
	struct cowfs_fsal_obj_handle *fh = NULL;
	bool created = false;
	...
	if (name == NULL) {
		return cowfs_open2_by_handle(obj_hdl, state, openflags,
	...
	dir_fd = cowfs_fsal_open(myself, O_PATH | O_NOACCESS, &status.major);
	 ...
	 */
	if (createmode != FSAL_NO_CREATE)
		if (!cowfs_set_credentials(op_ctx->creds, obj_hdl->fsal)) {
	...
	if (fd == -1 && errno == EEXIST && createmode == FSAL_UNCHECKED) {
		...
		if (createmode != FSAL_NO_CREATE)
			cowfs_restore_ganesha_credentials(obj_hdl->fsal);
	...
	/** @todo FSF: If we are running with ENABLE_CoWFS_DEBUG_ACL or a
	 *             CoWFS sub-FSAL that supports ACLs but doesn't permission
	...
	*caller_perm_check = createmode == FSAL_NO_CREATE;
	...
	cowfs_alloc_handle(fh);

	retval = cowfs_name_to_handle(dir_fd, obj_hdl->fs, name, fh);
	...
	*new_obj = &hdl->obj_handle;
	...
	if (created && attrib_set->valid_mask != 0) {
		 ...
		 *
		 * Note if we have ENABLE_CoWFS_DEBUG_ACL an inherited ACL might
...
fsal_status_t cowfs_reopen2(struct fsal_obj_handle *obj_hdl,
			  ...
			  fsal_openflags_t openflags)
{
	struct cowfs_fd fd, *my_fd = &fd, *my_share_fd;
	struct cowfs_fsal_obj_handle *myself;
	fsal_status_t status = {0, 0};
	int posix_flags = 0;
	fsal_openflags_t old_openflags;

	my_share_fd = &container_of(state, struct cowfs_state_fd, state)->fd;
...
{
	struct cowfs_fsal_obj_handle *myself;
	struct cowfs_filesystem *cowfs_fs;
	struct cowfs_fd temp_fd = {
			FSAL_O_CLOSED, PTHREAD_RWLOCK_INITIALIZER, -1 };
	struct cowfs_fd *out_fd = &temp_fd;
	fsal_status_t status = {ERR_FSAL_NO_ERROR, 0};
	int rc, posix_flags;
	bool reusing_open_state_fd = false;

	myself = container_of(obj_hdl, struct cowfs_fsal_obj_handle, obj_handle);
	cowfs_fs = myself->obj_handle.fs->private_data;
	...
	case CHARACTER_FILE:
	case BLOCK_FILE:
		rc = cowfs_open_by_handle(cowfs_fs,
	...
	case REGULAR_FILE:
		status = fsal_find_fd((struct fsal_fd **)&out_fd, obj_hdl,
				      ...
				      cowfs_open_func, cowfs_close_func,
	...
	rc = cowfs_fsal_open(myself, posix_flags, &status.major);
...
void cowfs_read2(struct fsal_obj_handle *obj_hdl,
	       bool bypass,
	       ...
	       void *caller_arg)
{
	 ...
	 */
	if (read_arg->state) {
		cowfs_fd = &container_of(read_arg->state, struct cowfs_state_fd,
					 state)->fd;

		PTHREAD_RWLOCK_rdlock(&cowfs_fd->fdlock);
	}
 ...
 out:
	...
	if (cowfs_fd)
		PTHREAD_RWLOCK_unlock(&cowfs_fd->fdlock);
...
void cowfs_write2(struct fsal_obj_handle *obj_hdl,
		bool bypass,
		fsal_async_cb done_cb,
		...
		void *caller_arg)
{
	 ...
	 */
	if (write_arg->state) {
		cowfs_fd = &container_of(write_arg->state, struct cowfs_state_fd,
				       state)->fd;
...
{
	struct cowfs_fsal_obj_handle *src_cowfs;
	struct cowfs_fsal_obj_handle *dst_cowfs;
	...
	LogDebug(COMPONENT_FSAL, "cowfs_copy: server-side copying");
	src_cowfs = container_of(src_hdl, struct cowfs_fsal_obj_handle, obj_handle);
	dst_cowfs = container_of(dst_hdl, struct cowfs_fsal_obj_handle, obj_handle);
	if ((size_t)src_cowfs < (size_t)dst_cowfs) {
	...
	st = find_fd(&src_fd, src_hdl, false, NULL, FSAL_O_READ,
                         &has_lock1, &need_fsync, &closefd1, false);
	if (FSAL_IS_ERROR(st)) {
                LogDebug(COMPONENT_FSAL,
	...
	st = find_fd(&dst_fd, dst_hdl, false, NULL, FSAL_O_WRITE,
                         &has_lock2, &need_fsync, &closefd2, false);
	if (FSAL_IS_ERROR(st)) {
                LogDebug(COMPONENT_FSAL,
                         "find_fd failed %s", msg_fsal_err(st.major));
                goto out;
	...
	*copied = ret;
	LogDebug(COMPONENT_FSAL, "cowfs_copy: %zu of %zu bytes copied", *copied,
		 count);
 ...
 out:
	...
	LogDebug(COMPONENT_FSAL, "Exited cowfs_copy");
	return st;
}
 ...
 *
 * For FSAL_CoWFS etc. we ignore owner, implicitly we have a lock_fd per
			   ...
			   fsal_lock_param_t *conflicting_lock)
{
	 ...
	 */
	if (state) {
		cowfs_fd = &container_of(state, struct cowfs_state_fd, state)->fd;
			   ...
			   struct attrlist *attrib_set)
{
	 ...
	 */
	if (state) {
		cowfs_fd = &container_of(state, struct cowfs_state_fd,
				       state)->fd;
	...
	if (FSAL_TEST_MASK(attrib_set->valid_mask, ATTR_MODE)) {
		 ...
		 */
		if (obj_hdl->type != SYMBOLIC_LINK) {
			if (cowfs_unopenable_type(obj_hdl->type))
				retval = fchmodat(
					my_fd,
...
fsal_status_t cowfs_close2(struct fsal_obj_handle *obj_hdl,
			 struct state_t *state)
{
	struct cowfs_fsal_obj_handle *myself = NULL;
	struct cowfs_fd *my_fd = &container_of(state, struct cowfs_state_fd,
					     state)->fd;
...
static int clone_file(const char *src, const char *dst)
{
        ...
        ret = src_fd = open(src, O_RDONLY);
        if (src_fd < 0) {
                LogDebug(COMPONENT_FSAL, "failed to open %s: ", src);
                perror("");
                goto end;
        }
        ...
        ret = stat(src, &sb);
        if (ret < 0) {
                LogDebug(COMPONENT_FSAL, "failed to stat");
                perror("");
                goto end;
        }

        ret = dest_fd = open(dst, O_WRONLY | O_CREAT, sb.st_mode & 07777);
        if (dest_fd < 0) {
                LogDebug(COMPONENT_FSAL, "failed to open %s: ", dst);
                perror("");
                goto end;
        }
        ...
        if (ret < 0) {
                LogDebug(COMPONENT_FSAL, "failed to clone %s: ", src);
                perror("");
        }
...
static int copy_metadata(const char *src, const char *dst)
{
        int ret;
        ...
        ret = lchown(dst, sb.st_uid, sb.st_gid);
        if (ret < 0)
                perror("lchown failed");

        if (S_ISDIR(sb.st_mode)) {
                ret = chmod(dst, sb.st_mode & 07777);
        }
end:
        return ret;
}
/* Allocates dst_name, needs to be free'd by caller */
fsal_status_t cowfs_clone(struct fsal_obj_handle *src_hdl,
			  char **dst_name,
			  struct fsal_obj_handle *dir_hdl,
			  char *file_uuid)
{
	char *clone_file_path, *name;
	int retval = 0;

	name = file_uuid;
	LogDebug(COMPONENT_FSAL, "cowfs_clone: server-side cloning");
	if (!dir_hdl) {
		LogDebug(COMPONENT_FSAL, "Null dir handle");
		goto err;
	}

	/* The cloned file has name /<tc_backup_dir>/<file_uuid as string> */
	clone_file_path = (char *) calloc(
					strlen(dir_hdl->absolute_path) +
					strlen(file_uuid) + 2,
					sizeof(char));
	if (!clone_file_path) {
		retval = ENOMEM;
	...
	strcpy(clone_file_path, dir_hdl->absolute_path);
	strcat(clone_file_path, "/");
	strcat(clone_file_path, name);

	if (access(clone_file_path, F_OK) != -1) {
		//file has been backed up already
		retval = EEXIST;
		free(clone_file_path);
		goto err;
	}
	PTHREAD_RWLOCK_rdlock(&src_hdl->obj_lock);
	/* Clone the file */
	retval = clone_file(src_hdl->absolute_path, clone_file_path);
	if (retval < 0) {
		LogDebug(COMPONENT_FSAL, "cowfs_clone: clone failed");
		goto out;
	}
	...
	*dst_name = clone_file_path;
	LogDebug(COMPONENT_FSAL, "Exited cowfs_clone success %s", *dst_name);	
	return fsalstat(ERR_FSAL_NO_ERROR, 0);

out:
	PTHREAD_RWLOCK_unlock(&src_hdl->obj_lock);
err:
	LogDebug(COMPONENT_FSAL, "Exited cowfs_clone");
	return fsalstat(posix2fsal_error(retval), retval);
}
