...
/* export.c
 * CoWFS Super-FSAL export object
 */
...
/* helpers to/from other CoWFS objects
 */
...
int cowfs_get_root_fd(struct fsal_export *exp_hdl)
{
	...
	struct cowfs_filesystem *my_root_fs;

	myself = EXPORT_CoWFS_FROM_FSAL(exp_hdl);
...
static void release(struct fsal_export *exp_hdl)
{
	...
	if (op_ctx != NULL && op_ctx->ctx_export != NULL) {
		LogDebug(COMPONENT_FSAL, "Releasing CoWFS export %"PRIu16
			 " for %s",
			 exp_hdl->export_id,
	...
	cowfs_sub_fini(myself);

	cowfs_unexport_filesystems(myself);
			       ...
			       fsal_quota_t *pquota)
{
	...
	if (!cowfs_set_credentials(op_ctx->creds, exp_hdl->fsal)) {
	...
	errsv = errno;
	cowfs_restore_ganesha_credentials(exp_hdl->fsal);
				    ...
				    int flags)
{
	...
	return cowfs_check_handle(exp_hdl, fh_desc, &fs, fh, &dummy);
}

/* cowfs_export_ops_init
 * overwrite vector entries with the methods that we support
 */

void cowfs_export_ops_init(struct export_ops *ops)
{
	ops->release = release;
	ops->lookup_path = cowfs_lookup_path;
	ops->wire_to_host = wire_to_host;
	ops->create_handle = cowfs_create_handle;
	ops->get_fs_dynamic_info = get_dynamic_info;
	ops->get_quota = get_quota;
	ops->set_quota = set_quota;
	ops->alloc_state = cowfs_alloc_state;
	ops->free_state = cowfs_free_state;
}

void free_cowfs_filesystem(struct cowfs_filesystem *cowfs_fs)
{
	if (cowfs_fs->root_fd >= 0)
		close(cowfs_fs->root_fd);
	gsh_free(cowfs_fs);
}

int cowfs_claim_filesystem(struct fsal_filesystem *fs, struct fsal_export *exp)
{
	struct cowfs_filesystem *cowfs_fs = fs->private_data;
	int retval;
	struct cowfs_fsal_export *myself;
	struct cowfs_filesystem_export_map *map;

	myself = EXPORT_CoWFS_FROM_FSAL(exp);

	map = gsh_calloc(1, sizeof(*map));

	if (fs->fsal != NULL) {
		cowfs_fs = fs->private_data;
		if (cowfs_fs == NULL) {
	...
	cowfs_fs = gsh_calloc(1, sizeof(*cowfs_fs));

	glist_init(&cowfs_fs->exports);
	cowfs_fs->root_fd = -1;

	cowfs_fs->fs = fs;

	retval = cowfs_get_root_handle(cowfs_fs, myself);
...
already_claimed:
	...
	map->fs = cowfs_fs;
	map->exp = myself;
	glist_add_tail(&cowfs_fs->exports, &map->on_exports);
...
errout:
	...
	if (cowfs_fs != NULL)
		free_cowfs_filesystem(cowfs_fs);

	return retval;
}

void cowfs_unclaim_filesystem(struct fsal_filesystem *fs)
{
	struct cowfs_filesystem *cowfs_fs = fs->private_data;
	...
	if (cowfs_fs != NULL) {
		glist_for_each_safe(glist, glistn, &cowfs_fs->exports) {
			...
			if (map->exp->root_fs == fs) {
				LogInfo(COMPONENT_FSAL,
					"Removing root_fs %s from CoWFS export",
					fs->path);
			}
...
void cowfs_unexport_filesystems(struct cowfs_fsal_export *exp)
{
	...
	glist_for_each_safe(glist, glistn, &exp->filesystems) {
		...
		if (glist_empty(&map->fs->exports)) {
			LogInfo(COMPONENT_FSAL,
				"CoWFS is no longer exporting filesystem %s",
				map->fs->fs->path);
				...
				const struct fsal_up_vector *up_ops)
{
	...
	myself = gsh_calloc(1, sizeof(struct cowfs_fsal_export));

	glist_init(&myself->filesystems);

	fsal_export_init(&myself->export);
	cowfs_export_ops_init(&myself->export.exp_ops);

	retval = load_config_from_node(parse_node,
				       cowfs_sub_export_param,
				       myself,
				       true,
				       err_type);
	if (retval != 0) {
	...
	cowfs_sub_init_export_ops(myself, op_ctx->ctx_export->fullpath);
	...
	retval = resolve_posix_filesystem(op_ctx->ctx_export->fullpath,
					  ...
					  cowfs_claim_filesystem,
					  cowfs_unclaim_filesystem,
					  &myself->root_fs);
	...
	retval = cowfs_sub_init_export(myself);
	if (retval != 0) {
		fsal_status = posix2fsal_status(retval);
		goto err_cleanup;
	}
